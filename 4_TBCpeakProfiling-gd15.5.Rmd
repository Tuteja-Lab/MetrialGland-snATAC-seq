...
ggplot(t, aes(x = Freq)) + geom_histogram(bins = 9, fill = "#c6dbef", color="grey") +
...
```{r}
t <- table(specificPeaks$gene_name)
t2 <- exp[intersect(names(t), rownames(exp)), c("23", "gene")]
temp <- data.frame("23"=rep(0, length(setdiff(names(t), rownames(exp)))),
                   gene=setdiff(names(t), rownames(exp)))
names(temp) <- c("23", "gene")
t2 <- rbind(t2, temp)
...
#write.table(t2, "/work/LAS/geetu-lab/hhvu/project3_scATAC/scATAC-seq-analysis/5_integrating2timepoints/annotation_scRNAseq/commonPeaks/gd15.5-tableS2.txt", sep = "\t", quote = F, row.names = F)
#cut -f 1-4,8,10 gd15.5-tableS2.txt | uniq -c > gd15.5-tableS2.2.txt
```

Candidate marker
```{r}
distinct(t2[t2$`23` > 1 & t2$marker == 0 & t2$noEVTpeaks >= 3 & t2$noRatPeaks >= 3 & !is.na(t2$Human.gene.name), c(1,2,3,4,8,10)])
```
