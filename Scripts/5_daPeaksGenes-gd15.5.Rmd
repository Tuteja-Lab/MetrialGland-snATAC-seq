...
From here, we focus on analyzing the peaks more accessible at GD15.5.

# Step 3: Peaks more accessibile at gd15.5 and motif analysis
The distribution of the DA peaks in the genome using GD15.5 coordinates. Since we have to merge objects of two timepoints together, the coordinates of the gd15.5-DA peaks may be slightly different from the original coordinates. We can map that back to the original coordinates with bedtools. In R:
```{r, eval=F}
...
file <- "/work/LAS/geetu-lab/hhvu/project3_scATAC/scATAC-seq-analysis/5_integrating2timepoints/annotation_scRNAseq/daPeaks/gd15.5TBC-DApeaks-original.bed"
...
peaks <- read.table("/work/LAS/geetu-lab/hhvu/project3_scATAC/scATAC-seq-analysis/5_integrating2timepoints/annotation_scRNAseq/daPeaks/gd15.5TBC-DApeaks-original.bed", header=F)
top.peaks <- peaks$V4
...
write.table(enriched.motifs, "/work/LAS/geetu-lab/hhvu/project3_scATAC/scATAC-seq-analysis/5_integrating2timepoints/annotation_scRNAseq/daPeaks/da15.5-ratsMotifs.txt", quote=F)
save(rats, file = "/work/LAS/geetu-lab/hhvu/project3_scATAC/scATAC-seq-analysis/5_integrating2timepoints/annotation_scRNAseq/daPeaks/da15.5_ATACwithRatPFM.rda")
```
Mouse motifs:
```{r, eval=F}
...
write.table(enriched.motifs, "/work/LAS/geetu-lab/hhvu/project3_scATAC/scATAC-seq-analysis/5_integrating2timepoints/annotation_scRNAseq/daPeaks/da15.5-mouseMotifs.txt", quote=F)
save(rats, file = "/work/LAS/geetu-lab/hhvu/project3_scATAC/scATAC-seq-analysis/5_integrating2timepoints/annotation_scRNAseq/daPeaks/da15.5_ATACwithMousePFM.rda")
```
Human motifs:
```{r, eval=F}
...
write.table(enriched.motifs, "/work/LAS/geetu-lab/hhvu/project3_scATAC/scATAC-seq-analysis/5_integrating2timepoints/annotation_scRNAseq/daPeaks/da15.5-humanMotifs.txt", quote=F)
save(rats, file = "/work/LAS/geetu-lab/hhvu/project3_scATAC/scATAC-seq-analysis/5_integrating2timepoints/annotation_scRNAseq/daPeaks/da15.5_ATACwithHumanPFM.rda")
```

Next, we compile the enriched motifs to one table.
```{r}
rat15.5 <- read.table("/work/LAS/geetu-lab/hhvu/project3_scATAC/scATAC-seq-analysis/5_integrating2timepoints/annotation_scRNAseq/daPeaks/da15.5-ratsMotifs.txt", header = T)
rat15.5$adj_pval <- p.adjust(rat15.5$pvalue, "BH")
rat15.5 <- rat15.5[rat15.5$adj_pval <= 0.05 & rat15.5$fold.enrichment >= 1.5,]
dim(rat15.5)

mouse15.5 <- read.table("/work/LAS/geetu-lab/hhvu/project3_scATAC/scATAC-seq-analysis/5_integrating2timepoints/annotation_scRNAseq/daPeaks/da15.5-mouseMotifs.txt", header = T)
mouse15.5$adj_pval <- p.adjust(mouse15.5$pvalue, "BH")
mouse15.5 <- mouse15.5[mouse15.5$adj_pval <= 0.05 & mouse15.5$fold.enrichment >= 1.5,]
mouse15.5$type <- "mouse"

human15.5 <- read.table("/work/LAS/geetu-lab/hhvu/project3_scATAC/scATAC-seq-analysis/5_integrating2timepoints/annotation_scRNAseq/daPeaks/da15.5-humanMotifs.txt", header = T)
human15.5$adj_pval <- p.adjust(human15.5$pvalue, "BH")
human15.5 <- human15.5[human15.5$adj_pval <= 0.05 & human15.5$fold.enrichment >= 1.5,]
human15.5$type <- "human"

motifs15.5 <- rbind(mouse15.5, human15.5)
write.table(motifs15.5, "/work/LAS/geetu-lab/hhvu/project3_scATAC/scATAC-seq-analysis/5_integrating2timepoints/annotation_scRNAseq/daPeaks/allMotifs-daPeaksGD15.5coor.txt", sep = "\t", quote = F)
```

Number of motifs
```{r}
length(motifs15.5$motif)
```

Next, filter and keep only transcription factors (TFs) with enriched motifs and have expression levels > 1 at GD15.5.
```{r}
library(dplyr)
...
enriched <- motifs15.5[, c("motif", "motif.name", "type")]
...
g2 <- rownames(exp15.5[rownames(exp15.5) %in% enriched2$TFgenes & exp15.5$`23` > 1,])

enriched2 <- enriched2[enriched2$TFgenes %in% g2,]

motifs15.5 <- motifs15.5[motifs15.5$motif.name %in% enriched2$motif.name,]
```
...
```{r}
load("/work/LAS/geetu-lab/hhvu/project3_scATAC/scATAC-seq-analysis/5_integrating2timepoints/annotation_scRNAseq/daPeaks/da15.5_ATACwithMousePFM.rda")
peaks <- read.table("/work/LAS/geetu-lab/hhvu/project3_scATAC/scATAC-seq-analysis/5_integrating2timepoints/annotation_scRNAseq/daPeaks/gd15.5TBC-DApeaks-original.bed", header=F)

temp <- motifs15.5[motifs15.5$type == "mouse",]
...
load("/work/LAS/geetu-lab/hhvu/project3_scATAC/scATAC-seq-analysis/5_integrating2timepoints/annotation_scRNAseq/daPeaks/da15.5_ATACwithHumanPFM.rda")

temp <- motifs15.5[motifs15.5$type == "human",]
for (i in 1:nrow(temp)) {
  p <- intersect(peaks$V4, names(which(GetMotifData(rats)[, temp$motif[i]] == 1)))
  temp1 <- data.frame(motif=rep(temp$motif[i], length(p)), 
                      peak=p)
...
Get genes upregulated at gd15.5 that are conserved in human EVT using scRNA-seq data, and these markers are also near to the DA peaks.
```{r}
marker15.5 <- read.table("/work/LAS/geetu-lab/hhvu/project3_scATAC/scATAC-seq-analysis/4_RNAintegration/scRNA-seqFiles/gd15.5-TBC-DEG.txt", header = T, sep = "\t")
markers <- intersect(rownames(marker15.5), new15.5$gene_name)
...
Get genes upregulated at gd15.5 using scRNA-seq data, and these markers are also near to the DA peaks.
```{r}
marker15.5 <- read.table("/work/LAS/geetu-lab/hhvu/project3_scATAC/scATAC-seq-analysis/4_RNAintegration/scRNA-seqFiles/gd15.5-TBC-DEG.txt", header = T, sep = "\t")
markers <- intersect(rownames(marker15.5), new15.5$gene_name)
...
```{r}
network <- inner_join(new15.5, motifPeaks, by = c("regions" = "peak"))
network <- inner_join(network, motifs15.5[,c("motif", "motif.name")], by = c("motif" = "motif"))
...
write.table(all, "/work/LAS/geetu-lab/hhvu/project3_scATAC/scATAC-seq-analysis/5_integrating2timepoints/annotation_scRNAseq/daPeaks/daPeaksGenes-gd15.5.txt", sep = "\t", row.names = F, quote = F)
```
