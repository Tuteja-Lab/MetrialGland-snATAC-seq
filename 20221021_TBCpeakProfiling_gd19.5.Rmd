...
ggplot(df, aes(x=category, y=log10(value+10^(-5)))) + geom_boxplot() + ggtitle("Cutoff = 3, TBC peaks all genes GD19.5")
...
4. Genes with >= 3 peaks, how many of them are markers:
```{r}
t2 <- t[t >= 3]
...
t3 <- exp[setdiff(names(t2), marker$gene), c("6", "gene")]
t3[is.na(t3$`6`), "6"] <- 0
...
write.table(t2, "/work/LAS/geetu-lab/hhvu/project3_scATAC/scATAC-seq-analysis/4_RNAintegration/2_MACS2Peaks/annotation_scRNAseq/gd19.5-tbc-genewithMore3peaks.txt", sep = "\t", quote = F, row.names = F)
```
...
```{r}
specificPeaks <- specificPeaks[order(specificPeaks$ensembl_gene_id),]
specificPeaks[specificPeaks$gene_name %in% c("Prl5a1", "Prl7d1"),]
```
...
wrap_elements(cov_plot & scale_fill_manual(values = cols) & xlab("Chromosome 17 position")) + ggtitle("GD19.5 Prl7d1") + theme(plot.title = element_text(size=18, face = "bold")) 
```
